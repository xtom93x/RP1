...
function kalendar(mesiac,rok) {
  ...
  var tab="<table border=1><thead><tr><th onclick='pred()'>&lsaquo;&lsaquo;</th><th colspan='5'>"+nazovMesiaca[mesiac]+" "+rok+"</th><th onclick='dal()'>&rsaquo;&rsaquo;</th></tr></thead><tbody><tr><th>Po</th><th>Ut</th><th>Str</th><th>Št</th><th>Pia</th><th>So</th><th>Ne</th></tr><tr>";
  var i=1;
  for(; i<denTyzn; i++) {
    tab+="<td></td>";
  }
  for(den=1; den<=pocetDniMesiaca[mesiac]; den++) {
    if(rok==aktual_r && mesiac==aktual_m && den==aktual_d) {
      tab+="<td style='background-color:green;'>"+den+"</td>";
    }
    else {
      tab+="<td ";
      if (i%7==0){
        tab+="style='background-color:gold;' ";
      }
